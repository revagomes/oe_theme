...
class NewsTickerAssert extends BasePatternAssert {
  ...
  protected function assertItems(array $expected_items, Crawler $crawler): void {
    ...
    self::assertCount(count($expected_items), $items, 'The expected news ticker items do not match the found news ticker items.');
    ...
    foreach ($expected_items as $index => $expected_item) {
      ...
      if (isset($expected_item['link'])) {
        ...
        $this::assertStringContainsString($expected_item['link'], $link->attr('href'));
        continue;
      }
      $this::assertStringContainsString($expected_item['content'], $actual_item->text());
    }
  }
